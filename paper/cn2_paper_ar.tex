...
L-BFGS، خاصّة في نظام الأبعاد المتوسطة. وهي غير حساسة لطول الوثبة $K_0$ وتمنح مفاضلة
نظيفة في $\tau$.

سيتوجّه العمل المستقبلي إلى: (ط) الانتقال إلى نظام البعد الكبير باستخدام حاصل الهيسيان
مع متجه (Hessian-vector products) بحيث لا تكون نيوتن الكاملة ضرورية، مع إبقاء بوابة
انحطاط نيوتن محسوبةً عن طريق حلاّء التدرجات المتقارنة؛ (ب) توطيد النظرية ذات الطورين
في~\ref{thm:checkpoint-ar} إلى معدّلٍ عالمي (غير محدَّب) عبر حدٍّ من نمط
Kurdyka--{\L}ojasiewicz، وتكميم كلفة الهيسيان عند كل نقطة لجعل التعقيد لكل $\varepsilon$
صرحاً؛ (ج) تنمية الحزمة المفتوحة المصدر (بنية تكامل مستمر وتغليف وواجهات GPU) المنشورة
مع هذا العمل.
