...
بينما ينجح $\cn$ بحوالي $\sim179$ هيسياناً.
\begin{figure}[H]
\centering
...
\subsection{الحساسية}
غيّرنا طول الوثبة $K_0\in\{5,10,20,40\}$ على Rosenbrock $n{=}100$: فالنتيجة متطابقة
($f=0$، $\sim262$ هيسياناً) في كل الحالات (الشكل~\ref{fig:k0-ar}). أما تغيير العتبة $\tau$
فيكشف مفاضلةً نظيفة: فكلّما صغُرت $\tau$ زادت الدقة وارتفع عدد الهيسيانات
(الشكل~\ref{fig:tau-ar}).

\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{figures/sensitivity_K0.png}
\caption{متانة الطريقة على طول الوثبة المحدد من قبل المستخدم $K_0$.}
\label{fig:k0-ar}
\end{figure}
...
العدّادات $260/262/264$ لـ $\mathtt{newton\_steps}=1,2,3$؛ وعلى Beale $20/21/22$؛ وعلى
Himmelblau $5/7/9$ (الشكل~\ref{fig:e6-ar}). كل خطوة نيوتن إضافية تكلف هيسياناً واحداً على
...
وارithmos 3 مقابل 8 ($d{=}65$) من \textenglish{\texttt{scikit-learn}}. تبلغ كلُّ الطرق المثالةَ
...
ويتقارب $\cn$ بمتانةٍ إلى نفس المثالية بعدد $81$ و$77$ هيسياناً. القراءة الصادقة هي أن اقتصادية
...
على المربع ذي $\kappa{=}10^4$ في $n=1000$ يبلغ $\cn$-HVP
$f=4.4{\times}10^{-17}$ بـ $60$ تكرار CG فقط في $0.50\,$s، مقابل Newton-CG
($2996$ CG، $0.08\,$s) وL-BFGS ($f=3.0{\times}10^{-11}$، $2000$ تكرار). فالنُسخة
المتدفقة أرخصُ $50\times$ في العمل الخطي من Newton-CG مع مطابقة دقّتها. وعلى عائلة
Rosenbrock غير المحدّبة عند $n=500$ و$n=1000$، $\cn$-HVP \textbf{يتقارب إلى الدقة
الآلية} ($f=2.9{\times}10^{-16}$ و$f=2.0{\times}10^{-16}$ بـ $29$ تكرار CG لكل
حالة) بينما يعلّق Newton-CG عند $f\approx 3.5{\times}10^2$ و$f\approx 8.4{\times}10^2$
($5041$ تكرار CG) وتفشل L-BFGS ($f=3.0{\times}10^3$ و$f=7.0$). هذا يمثّل خفضاً بـ
$174\times$ في عمل CG على أصعب حالة اختبار (Rosenbrock $n=1000$) ويبرهن أن مسبار
لانكزوس خفيف الوزن ($25$ HVP لكل نقطة فحص) يجعل بوابة انحطاط نيوتن آمنةً للأنظمة
غير المحدّبة ذات البعد الكبير.
