...
$f=1.6{\times}10^{-22}$. On \emph{Rosenbrock} $n=200$, New\-ton consumes $300$ Hessians yet
  fails, while $\cn$ succeeds with $\sim178$ Hessians.
...
The user-defined cadence $K_0$ sets the effective re-check period
$K_{\mathrm{ck}} = \max(K_0,\, n/5)$ of the dual gate. Varying $K_0$ on
Rosenbrock $n{=}100$ (where the dimension floor is $n/5=20$): any $K_0\le20$
gives identical runs ($f=0$, $\sim261$ Hessians), while $K_0=50$ and $K_0=100$
raise the period and trade fewer Hessian/probe evaluations ($108$, resp.\ $57$)
for longer Nesterov bursts, still reaching $f=0$ (\cref{fig:k0}). Varying the
threshold $\tau$ exposes a clean trade-off: smaller $\tau$ yields higher
accuracy but more Hessians (\cref{fig:tau}).

\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{figures/sensitivity_K0.png}
\caption{Effect of the gate cadence $K_0$ on Rosenbrock $n{=}100$: values at
or below the dimension floor $n/5$ are identical; larger values trade fewer
gate probes for longer Nesterov bursts, $f=0$ in all cases.}
\label{fig:k0}
\end{figure}
...
\section{Implications of the Extended Experiments}
\label{sec:implications}

We ran four further experiment families after the initial version, each
targeting a claim of the theory or the roadmap.

\subsection{Conditioning and dimension invariance (E1)}
On the strongly convex quadratic family
$f(x)=\sum_i \kappa^{\frac{i-1}{n-1}} x_i^2$ the economy result
$\#\text{Hessians}=O(1)$ is sharp: both $\cn$ and Newton solve every instance
in exactly \textbf{3} Hessian evaluations, independent of $\kappa\in\{10^2,
10^4, 10^6\}$ and $n\in\{10,50,100,500\}$ (\cref{fig:e1}). A single Newton
step solves a quadratic exactly, and the flexible gate correctly keeps the
method in a Newton-only regime, so {\em neither} solver wastes Hessians on the
easiest problem class. The practical content of E1 is robustness, not speedup:
parameter grids over $\kappa$ and $n$ cannot break the phase structure.

\begin{figure}[H]
\centering
\includegraphics[width=1.0\linewidth]{figures/extended_heatmaps.png}
\caption{E1: Hessian evaluations of $\cn$ (left) and Newton (right) on
QuadIllCond. Both are 3 on every cell of the $n\times\kappa$ grid.}
\label{fig:e1}
\end{figure}

\subsection{Insensitivity to the number of Newton steps per cycle (E6)}
Removing the second Newton step of each cycle does not change the results:
on Rosenbrock $n{=}100$ the Hessian counts are $181/261/263$ for
$\texttt{newton\_steps}=1,2,3$; on Beale $25/26/27$; on Himmelblau $6/8/9$
(\cref{fig:e6}). Each extra Newton step costs at most one additional Hessian
per cycle, and the dual gate absorbs the rest of the modification. This
validates the architecture choice of the roadmap and lowers the tuning burden.

\begin{figure}[H]
\centering
\includegraphics[width=0.72\linewidth]{figures/ablation_newton_steps.png}
\caption{E6: Hessian economy is flat in the number of Newton steps per cycle.}
\label{fig:e6}
\end{figure}

\subsection{Two-phase trajectory (E7)}
\Cref{fig:e7} renders the entry measure $\lambda(x)$ (Newton decrement, red
markers) or $\|g\|$ (gradient norm, blue markers) at every cycle boundary for
Beale, started in a nonconvex region, and for Rosenbrock $n=100$. On Beale the
early checkpoints use the gradient-norm gate (nonconvex transit phase,
Theorem~\ref{thm:checkpoint} phase II), then switch to the Newton-decrement
gate (convex phase, phase I) before termination. On Rosenbrock the region is
convex along the whole run, so only the $\lambda$-gate fires. The figure is
the first direct visualization of the gate's geometric behavior claimed in
\cref{sec:dual-gate}.
...
\caption{E7: entry measure along checkpoints for Beale (top) and Rosenbrock
$n{=}100$ (bottom), colored by which gate fired.}
\label{fig:e7}
\end{figure}

\subsection{Real datasets (E2)}
We fit L2-regularized binomial logistic regression on the UCI breast-cancer
($d{=}31$) and digits 3-vs-8 ($d{=}65$) data from \texttt{scikit-learn}. All
methods reach the identical optimum ($f=5.98{\times}10^{-2}$ and
$f=1.42{\times}10^{-2}$, \cref{tab:e2}); the log-loss is convex, so the naive
Newton method is expectedly best in Hessian count ($10$ and $11$) and
L-BFGS needs none. $\cn$ converges robustly to the same optimum with $80$ and
$76$ Hessians. The honest reading is that $\cn$'s Hessian economy is a property
of the {\em nonconvex} regime, where Newton and quasi-Newton fail; on
strongly convex log-loss there is nothing to save.

\begin{table}[H]
\centering
\caption{Real-data logistic regression: final $f$, Hessian count.}
\label{tab:e2}
\small
...
\subsection{Hessian-free large-$n$ regime (ROADMAP Bk. 4)}
We implemented Hessian--vector products
(\texttt{src/hvp\_cg.py}: \texttt{hvp\_fd}, \texttt{HvpOperator},
\texttt{cg\_solve}) and a Newton-CG inner solver that preserves the
Newton-decrement gate without ever forming $H$. To make the dual gate reliable
on nonconvex problems, we added a \textbf{limited Lanczos negative-curvature
detector} (\texttt{lanczos\_min\_eig}, $k=25$ steps) that rejects the
Newton-decrement gate whenever the minimum eigenvalue of $H+\epsilon I$ falls
below $-10^{-2}$. This closes the streaming gap on the Rosenbrock family.

We report the substitute work \emph{honestly}: we split the HVP count into
$\mathrm{hv}$ (Newton-direction CG iterations) and $\mathrm{hv}_{\rm gate}$
(per-check gate cost: $6$ SPD-probe HVPs $+$ up to $25$ Lanczos HVPs $+$ one
$\lambda$-gate CG solve). On the $\kappa{=}10^4$ quadratic in $n=1000$,
$\cn$-HVP reaches $f=4.4{\times}10^{-17}$ in $60$ Newton-direction CG
iterations but $\mathrm{hv}_{\rm total}=4595$ once the gate is included,
vs.\ $2996$ CG iterations for Newton-CG; the gate probe is the price of keeping
$\lambda$ streaming without forming $H$. On the \emph{nonconvex} Rosenbrock
family at $n=500$ and $n=1000$, $\cn$-HVP \textbf{converges to machine
precision} ($f\approx4.7{\times}10^{-16}$ and $f\approx1.6{\times}10^{-16}$)
in $27$/$29$ Newton-direction CG iterations plus $4544$/$4353$ gate HVPs
($\mathrm{hv}_{\rm total}=4571/4382$), whereas Newton-CG stalls at
$f\approx3.5{\times}10^2$ and $f\approx8.4{\times}10^2$ ($5041$ CG iterations)
and L-BFGS fails ($f\approx3.0{\times}10^3$ and $f\approx7.0$). Within the
Newton-direction CG count this is a $174\times$ reduction on Rosenbrock
$n=1000$; including the gate then honest total is $4382$ vs.\ $5041$ HVPs,
which remains below Newton-CG while converging where Newton-CG fails
altogether. The honest lesson is that in the streaming regime the per-check
Lanczos probe is no longer free: the gate's benefit is accuracy and robustness
(converging where Newton-CG stalls) rather than raw HVP count.

The open problem documented in the earlier version — the $\lambda{=}0$ artifact
from a single-direction Rayleigh probe — is now resolved by the multi-direction
probe combined with the Lanczos safeguard. The method is ready for
moderate-to-large scale nonconvex problems.
...
moderate-dimensional regime. It is insensitive to the jump length $K_0$, to the number of
Newton steps per cycle, and offers a clean $\tau$ trade-off; per-cycle Hessian cost is
quantified in the supplementary note (complexity per-$\varepsilon$ for both phases).
The Hessian-free extension with a limited-Lanczos negative-curvature detector extends this
reliability to the large-$n$ nonconvex regime.

Future work will (i)~tighten the two-phase theory of \cref{thm:checkpoint} to a global
(nonconvex) rate via a Kurdyka--{\L}ojasiewicz-type bound; (ii)~grow the
open-source package (CI, packaging, GPU backends) released with this work; and
(iii)~explore adaptive-$k$ Lanczos schedules for further efficiency.
