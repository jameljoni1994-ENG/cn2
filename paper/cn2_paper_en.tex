...
Future work will (i)~move to the large-$n$ regime using Hessian-vector
products so that full Newton is unnecessary, while preserving the
Newton-decrement gate through a conjugate-gradient solver;
(ii)~tighten the two-phase theory of \cref{thm:checkpoint} to a global
(nonconvex) rate via a Kurdyka--{\L}ojasiewicz-type bound, and quantify the
per-checkpoint Hessian cost so the complexity-per-$\varepsilon$ is explicit;
and (iii)~grow the open-source package (CI, packaging, GPU backends) released
with this work.
