...
on Rosenbrock $n{=}100$ the Hessian counts are $260/262/264$ for
$\texttt{newton\_steps}=1,2,3$; on Beale $20/21/22$; on Himmelblau $5/7/9$
...
L-BFGS needs none. $\cn$ converges robustly to the same optimum with $81$ and
$77$ Hessians. The honest reading is that $\cn$'s Hessian economy is a property
...
On the $\kappa{=}10^4$ quadratic in $n=1000$, $\cn$-HVP reaches
$f=4.4{\times}10^{-17}$ with only $60$ CG iterations in $0.50$\,s, versus
Newton-CG ($2996$ CG, $0.08$\,s) and L-BFGS ($f=3.0{\times}10^{-11}$, $2000$
iters). The streaming version is therefore $50\times$ cheaper in linear-algebra
work than Newton-CG while matching its accuracy. On the \emph{nonconvex}
Rosenbrock family at $n=500$ and $n=1000$, $\cn$-HVP \textbf{converges to
machine precision} ($f=2.9{\times}10^{-16}$ and $f=2.0{\times}10^{-16}$ with
$29$ CG iterations each) whereas Newton-CG stalls at $f\approx 3.5{\times}10^2$
and $f\approx 8.4{\times}10^2$ ($5041$ CG iterations) and L-BFGS fails
($f=3.0{\times}10^3$ and $f=7.0$). This represents a $174\times$ reduction in
CG work on the hardest test case (Rosenbrock $n=1000$) and demonstrates that
a lightweight Lanczos probe ($25$ HVPs per checkpoint) makes the
Newton-decrement gate safe for nonconvex large-$n$ streaming.
